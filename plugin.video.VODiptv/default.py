...
def m3uCategory(url, logos, cache):
    ...
    for channel in list:
        ...
        AddDir(name, url, 3, image, meta, isFolder=False)
...
def AddDir(name, url, mode, iconimage, meta, logos="", index=-1, move=0, isFolder=True, background=None, cacheMin='0'):
...
#    meta = metacheck('movie', name,'','','2017')
    if meta != '':
        background = str(meta['backdrop_url']).replace('u', '')
