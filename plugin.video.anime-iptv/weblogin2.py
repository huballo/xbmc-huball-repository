...
def doLogin(cookiepath, username, password):
    ...
    if not os.path.isfile(cookiepath):
        ...
        cookiepath = os.path.join(cookiepath,'cookiesDrama.lwp')
    ...
    if username and password:
        ...
        login_url = 'http://www.dramaqueen.pl/profile/'

        #the header used to pretend you are a browser
        header_string = 'Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.9.2.18) Gecko/20110621 Mandriva Linux/1.9.2.18-0.1mdv2010.2 (2010.2) Firefox/3.6.18'

	#build the form data necessary for the login
        login_data = urllib.urlencode({'log':username, 'pwd':password, 'wp-submit':'Zaloguj się'})
