...
##########
iconFav = xbmcaddon.Addon(id="plugin.video.anime-iptv-2.0").getAddonInfo('path') + '/art/favorites.png'
iconSite = xbmcaddon.Addon(id="plugin.video.anime-iptv-2.0").getAddonInfo('path') + '/art/icon.png'
fanartSite = xbmcaddon.Addon(id="plugin.video.anime-iptv-2.0").getAddonInfo('path') + '/art/japan/fanart.jpg'
            ...
            if favs:
                for (_name, _year, _img, _fanart, _country, _url, _plot, _Genres, _site, _subfav, _section, _ToDoParams, _commonID, _commonID2) in favs:
                    if (name == _name):
                        ...
                        db.commit()
                        tf = True
                        myNote(bFL(name.upper()), bFL((lang(30003))))
                        xbmc.executebuiltin("Container.Refresh")
                if (tf == False):
                    myNote(bFL(name.upper()), bFL((lang(30004))))
            else:
                myNote(bFL(name.upper() + '  (' + year + ')'), bFL((lang(30004))))
    except Exception as e:
        db.rollback()
        raise e
    finally:
...
def fav__COMMON__add(site, section, name, year='', img=_artIcon, fanart=_artFanart, subfav='', plot='', commonID='', commonID2='', ToDoParams='', Country='', Genres='', Url=''):
    ...
    try:
                ...
                if favs:
                    for (_name, _year, _img, _fanart, _country, _url, _plot, _Genres, _site, _subfav, _section, _ToDoParams, _commonID, _commonID2) in favs:
                        if (name == _name) and (year == _year):
                            if len(year) > 0:
                                myNote(bFL(section + ':  ' + name.upper() + '  (' + year + ')'), bFL((lang(30005))))
                            else:
                                myNote(bFL(section + ':  ' + name.upper()), bFL((lang(30005))))
                            return
                favs.append((name, year, img, fanart, Country, Url, plot, Genres, site, subfav, section, ToDoParams, commonID, commonID2))
                favs = str(favs)
                cur.execute('UPDATE pluginvideoanimeiptv SET data = ? WHERE name = ? ', (favs, saved_favs))
        ...
        db.close()
    if len(year) > 0:
        myNote(bFL(name + '  (' + year + ')'), bFL((lang(30002))))
    else:
        myNote(bFL(name), bFL((lang(30002))))
