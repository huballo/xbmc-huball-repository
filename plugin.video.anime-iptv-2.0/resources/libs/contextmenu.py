...
from addon import Addon  # może trzeba więcej
import favourites
#######
_addon = Addon('plugin.video.anime-iptv-2.0', sys.argv)
...
def ContextMenu_Movies(labs={}):
    ...
    if tfalse(addst("CMI_ShowInfo")) == True:
        contextMenuItems.append(('Movie Info', 'Action(Info)'))
    if labs == {}:
        return contextMenuItems
    if (tfalse(addst("CMI_SearchKissAnime")) == True) and (os.path.exists(translatePath("special://home/addons/") + 'plugin.video.kissanime')):
        contextMenuItems.append(('Search KissAnime', 'Container.Update(%s?mode=%s&pageno=1&pagecount=1&title=%s)' % ('plugin://plugin.video.kissanime/', 'Search', nameonly)))
    if (tfalse(addst("CMI_SearchSolarMovieso")) == True) and (os.path.exists(translatePath("special://home/addons/") + 'plugin.video.solarmovie.so')):
        contextMenuItems.append(('Search Solarmovie.so', 'Container.Update(%s?mode=%s&section=%s&title=%s)' % ('plugin://plugin.video.solarmovie.so/', 'Search', 'movies', nameonly)))
    if (tfalse(addst("CMI_Search1Channel")) == True) and (os.path.exists(translatePath("special://home/addons/") + 'plugin.video.1channel')):
        contextMenuItems.append(('Search 1Channel', 'Container.Update(%s?mode=7000&section=%s&query=%s)' % ('plugin://plugin.video.1channel/', 'movies', nameonly)))
    try:
        WRFC = (lang(30001))
        LB = DoLabs2LB(labs)
...
def ContextMenu_Series(labs={}):
    ...
    try:
        ...
        section = addpr('section', '')
    if tfalse(addst("CMI_ShowInfo")) == True:
        contextMenuItems.append(('Show Info', 'Action(Info)'))
    if labs == {}:
        return contextMenuItems
    try:
        WRFC = (lang(30001))
    ...
    LB = DoLabs2LB(labs)
    LB['mode'] = 'addView'
    Pars = 'RunPlugin(%s)' % _addon.build_plugin_url(LB)
...
def ContextMenu_Episodes(labs={}):
    contextMenuItems = []
#    contextMenuItems.append(('Watched/Unwatched', 'Action(ToggleWatched)'))
    if labs == {}:
        return contextMenuItems
    return contextMenuItems
...
def ContextMenu_Favorites(labs={}):
    ...
    try:
        ...
        LB['subfav'] = _subfav
        Pars = P1 % _addon.build_plugin_url(LB)
        contextMenuItems.append(((lang(30006)) + WRFC + addst('fav.tv.' + _sf + '.name'), Pars))
    except:
        pass
    return contextMenuItems